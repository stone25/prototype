...
if($connexion->connecter())
{
    ...
    $req = "select nom, prenom, contact, sexe, photo, denomination, statut, ((dayofyear(date) - dayofyear(NOW())) + 1) as cpt1, (dayofyear(date) + 365 - dayofyear(NOW())) as cpt2  from jeciste inner join militer on jeciste.id = militer.idjeciste inner join instance on militer.idinstance = instance.id where ((dayofyear(date) - dayofyear(NOW())) + 1) BETWEEN 1 and 10 OR dayofyear(date) + 365 - dayofyear(NOW()) BETWEEN 1 and 10 order by cpt1 asc";
    $fic1 = mysqli_query($connexion->connexion, $req) or die(mysqli_error($connexion->connexion));
    
    $req = "select nom, prenom, contact, sexe, photo, denomination, statut, (year(now())-year(date)) as age  from jeciste inner join militer on jeciste.id = militer.idjeciste inner join instance on militer.idinstance = instance.id where ((dayofyear(date) - dayofyear(NOW())) + 1) = 0 OR dayofyear(date) + 365 - dayofyear(NOW()) = 0 order by nom asc";
    $fic2 = mysqli_query($connexion->connexion, $req) or die(mysqli_error($connexion->connexion));

    $req = "select count(*) as cpt from jeciste inner join militer on id = idjeciste";
    $fic = mysqli_query($connexion->connexion, $req) or die(mysqli_error($connexion->connexion));
    $nb_jeciste=mysqli_fetch_assoc($fic);

    $req = "select count(*) as cpt from jeciste inner join militer on id = idjeciste";
    $fic = mysqli_query($connexion->connexion, $req) or die(mysqli_error($connexion->connexion));
    $nb_jeciste=mysqli_fetch_assoc($fic);

    $req = "select count(*) as cpt from jeciste j inner join commander co on j.id = co.idjeciste inner join commande c on c.id = co.idcommande";
    $fic = mysqli_query($connexion->connexion, $req) or die(mysqli_error($connexion->connexion));
    $nb_carte=mysqli_fetch_assoc($fic);
}
                        ...
                        <div class="hpanel widget-int-shape responsive-mg-b-30">
                            <div class="panel-body">
                                ...
                                <div class="m-t-xl widget-cl-1">
                                    <h1 class="text-success"><?php $symp = $nb_jeciste['cpt']-$nb_carte['cpt']; echo "$nb_carte[cpt]/$symp"; ?></h1>
                                    <small>
										Jecistes/sympathisants.	
									</small>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                        <div class="hpanel widget-int-shape responsive-mg-b-30">
                            <div class="panel-body">
                                ...
                                <div class="m-t-xl widget-cl-1">
                                    <h1 class="text-success"><?php echo "$respo[cpt]/$jeciste[cpt]"; ?></h1>
                                    <small>
										Responsables/Jecistes
									</small>
                                </div>
                            </div>
                        </div>
